...
* As of writing this, i'm recreating the Companion character with different code and i got THREE problems: 1 their hurtbox doesn't work, 2 their swimming is broken (One issue i see if when they quit SWIMMING and return to it, they stop chasing the Player horzintally; Another SWIMMING issue is when they're close to you and if you make them switch to SWIMIDLE, they go up really fast and jump to the water surface) and 3 their climbing is a mess because they shake really hard if the Player is close enough. The reason why this recreation exists is because the Companion iteration i used before had complicated code i had to eventually abandon (And the use of OS.get_ticks_msec() could bring future problems)
